...
#else
        QString path = QStandardPaths::standardLocations(QStandardPaths::CacheLocation).at(0);
#endif
    QDir dir(path);
...
#else
void Utility::setEngine(QQmlEngine *engine)
#endif
{
    this->engine = engine;
}
...
#else
    QString path = QStandardPaths::standardLocations(QStandardPaths::DataLocation).at(0) + QDir::separator() + ".userdata";
#endif
    QDir dir(path);
    if (!dir.exists()) dir.mkpath(path);
...
QString Utility::decodeGBKHex(const QString &encodedString)
{
    ...
    QByteArray text = QByteArray::fromHex(encodedString.toLatin1());
...
#elif Q_OS_SAILFISH
    QString path("file:///usr/share/harbour-tbclient/");
#else
    QString path("file:///");
...
#elif Q_OS_SAILFISH
    QFile file("/usr/share/harbour-tbclient/qml/emo/custom.dat");
#else
...
#elif Q_OS_SAILFISH
    QFile file("/usr/share/harbour-tbclient/qml/emo/emo.dat");
#else
